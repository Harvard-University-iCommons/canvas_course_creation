...
from bulk_site_creation.utils import (
...
class UtilsTest(TestCase):
    def setUp(self):
        ...
        self.colgsas_template_context_data = [{
            ...
            "canvas_course_url": "https://canvas.icommons.harvard.edu/courses/6066"
        }]
    ...
    @patch('icommons_common.canvas_api.helpers.accounts.get_department_accounts')
    @patch('icommons_common.canvas_api.helpers.accounts.get_account_by_sis_account_id')
    def test_get_department_data_for_school(self, mock_get_account_by_sis_account_id, mock_get_accounts):
        mock_get_account_by_sis_account_id.return_value = self.school_account
        mock_get_accounts.return_value = self.department_accounts
        result = get_department_data_for_school(self.canvas_user_id, self.school_sis_account_id)
    ...
    @patch('icommons_common.canvas_api.helpers.accounts.get_department_accounts')
    @patch('icommons_common.canvas_api.helpers.accounts.get_account_by_sis_account_id')
    def test_get_department_data_for_school_single(self, mock_get_account_by_sis_account_id, mock_get_accounts):
        mock_get_account_by_sis_account_id.return_value = self.school_account
        mock_get_accounts.return_value = self.department_accounts
    ...
    @patch('icommons_common.canvas_api.helpers.accounts.get_course_group_accounts')
    @patch('icommons_common.canvas_api.helpers.accounts.get_account_by_sis_account_id')
    def test_get_course_group_data_for_school(self, mock_get_account_by_sis_account_id, mock_get_accounts):
        mock_get_account_by_sis_account_id.return_value = self.school_account
        mock_get_accounts.return_value = self.course_group_accounts
        result = get_course_group_data_for_school(self.canvas_user_id, self.school_sis_account_id)
    ...
    @patch('icommons_common.canvas_api.helpers.accounts.get_course_group_accounts')
    @patch('icommons_common.canvas_api.helpers.accounts.get_account_by_sis_account_id')
    def test_get_course_group_data_for_school_single(self, mock_get_account_by_sis_account_id, mock_get_accounts):
        mock_get_account_by_sis_account_id.return_value = self.school_account
        mock_get_accounts.return_value = self.course_group_accounts
    ...
    @patch('bulk_site_creation.utils.get_all_list_data')
    @patch('bulk_site_creation.utils.CanvasSchoolTemplate.objects.filter')
